...
class NutriScanAISupabaseUser extends BaseAuthUser {
  NutriScanAISupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> nutriScanAISupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NutriScanAISupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
